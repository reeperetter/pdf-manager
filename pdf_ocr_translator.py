...
PDF OCR & Translator
=====================
Програма з GUI (Tkinter) для Python 3.12.

Що робить:
1. Приймає один або декілька PDF-файлів (багатосторінкових).
2. Розпізнає текст на кожній сторінці (EasyOCR — чистий Python, БЕЗ окремих
   програм на кшталт Tesseract; встановлюється звичайним pip install).
3. Створює "розпізнаваний" PDF: виглядає так само, як оригінал (та сама
   картинка), але текст можна виділяти / копіювати / шукати (невидимий
   текстовий шар).
4. (Головна функція) Перекладає розпізнаний текст на українську та/або
   російську мову і зберігає ОКРЕМИЙ PDF, де перекладений текст вставлено
   ПРЯМО НА МІСЦІ оригінального тексту (макет і картинки зберігаються,
   текст замінено).

Залежності (встановити через pip, більше НІЧОГО окремо ставити не треба):
    pip install -r requirements.txt

Перший запуск для кожної обраної мови розпізнавання завантажить
модель EasyOCR (кілька десятків МБ, один раз, потім працює офлайн).
Переклад (Google Translate) потребує інтернету завжди.

Автор: згенеровано Claude (Anthropic) на замовлення користувача.
"""
...
# =========================================================================
#  Шрифт з кирилицею — постачається разом зі скриптом (DejaVuSans.ttf),
#  щоб програма працювала "з коробки" на будь-якому комп'ютері.
# =========================================================================
...
def process_pdf(
    reader,               # готовий easyocr.Reader (створюється один раз на сесію)
    input_path,
    output_dir,
    dpi,
    make_searchable,
    ...
    for page_index in range(n_pages):
        ...
        log_fn(f"[{base_name}] Сторінка {page_index + 1}/{n_pages}: розпізнавання тексту...")
        ...
        if out_searchable is not None:
            ...
            for line in lines:
                ...
                if rect.is_empty or rect.width <= 0 or rect.height <= 0:
                    continue
                fs = fit_fontsize(line["text"], rect.width, rect.height, fontfile)
        ...
        if translate_targets and lines:
            log_fn(f"[{base_name}] Сторінка {page_index + 1}/{n_pages}: переклад...")
            ...
            for lang in translate_targets:
                try:
                    ...
                    log_fn(f"  [!] Помилка перекладу ({lang}): {e}. Пробую по рядку...")
                    result = []
            ...
            for lang in translate_targets:
                ...
                for line, tr_text in zip(lines, translated_lines):
                    ...
                    if not tr_text:
                        continue
                    fs = fit_fontsize(tr_text, rect.width, rect.height, fontfile)
                    baseline = fitz.Point(rect.x0, rect.y1 - 0.15 * fs)
                    try:
                        tp.insert_text(
                            baseline,
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.last_dir = "C:\\" if os.name == "nt" else "/"
    ...
    def _build_ui(self):
        ...
        ttk.Button(btns, text="Видалити вибране", command=self.remove_selected).pack(fill="x", pady=2)
        ttk.Button(btns, text="Очистити список", command=self.clear_files).pack(fill="x", pady=2)
        ...
        ttk.Label(frm_opts, text="DPI рендерингу:").grid(row=2, column=0, sticky="w", **pad)
        ...
        ttk.Button(frm_opts, text="Огляд...", command=self.choose_font).grid(row=4, column=3, sticky="w", **pad)
        ...
        ttk.Button(frm_out, text="Огляд...", command=self.choose_out_dir).pack(side="left", padx=6)

        frm_run = ttk.Frame(self)
        frm_run.pack(fill="x", **pad)
        self.start_btn = ttk.Button(frm_run, text="Почати обробку", command=self.start_processing)
        self.start_btn.pack(side="left", padx=6)
        self.cancel_btn = ttk.Button(frm_run, text="Скасувати", command=self.cancel_processing, state="disabled")
    ...
    def add_folder(self):
        folder = filedialog.askdirectory(title="Виберіть папку з PDF-файлами", initialdir=self.last_dir)
        if folder:
    ...
    def choose_out_dir(self):
        folder = filedialog.askdirectory(title="Папка для результатів", initialdir=self.last_dir)
        if folder:
    ...
    def start_processing(self):
        if not self.files:
            messagebox.showwarning("Немає файлів", "Спочатку додайте хоча б один PDF-файл.")
            return
        if not (self.var_searchable.get() or self.var_uk.get() or self.var_ru.get()):
            messagebox.showwarning("Нічого робити", "Виберіть хоча б одну дію у розділі 3.")
            return

        ocr_langs = [code for code, var in self.ocr_lang_vars.items() if var.get()]
        if not ocr_langs:
            messagebox.showwarning("Немає мов", "Виберіть хоча б одну мову розпізнавання у розділі 2.")
            return
    ...
    def _get_reader(self, ocr_langs, gpu):
        ...
        if key not in self.reader_cache:
            ...
            self.reader_cache[key] = easyocr.Reader(list(ocr_langs), gpu=gpu, verbose=False)
    ...
    def _run_worker(self, files, out_dir, ocr_langs, gpu, dpi, make_searchable, translate_targets, font_path):
        try:
            ...
            self.log(f"[ПОМИЛКА] Не вдалося завантажити модель розпізнавання:\n{traceback.format_exc()}")
            self.start_btn.after(0, lambda: self.start_btn.config(state="normal"))
            self.cancel_btn.after(0, lambda: self.cancel_btn.config(state="disabled"))
            return
        ...
        for path in files:
            ...
            self.progress.after(0, lambda d=done: self.progress.config(value=d))
